...
export type Config = {
  ...
  daysPerWeek?: number      // training frequency chosen at onboarding (V2 uses a number, not a range label)
  email?: string
  heightCm?: number
  dob?: string               // ISO date
  restTimerSeconds?: number  // default rest between sets, used by ActiveSessionView
...
// metrics), each optionally auto-computed from real data via `linkedMetric`.

export type LinkedMetric = 'weight' | 'streak' | 'bodyFat' | 'manual'
export type Goal = {
  id: string
  title: string
  category: string
  unit: string
  dir: 'up' | 'down'          // whether progress means the number increasing or decreasing
  start: number
  target: number
  current: number             // authoritative only when linkedMetric === 'manual'; else recomputed live
  deadline?: string           // ISO date
  color: string
  linkedMetric: LinkedMetric
  linkedExercise?: string     // when linkedMetric would be 'lift' in future — reserved
  completed?: boolean
  createdAt: string
}

export function getGoalsList(): Goal[] { return load<Goal[]>('rj_goals_list', []) }
export function saveGoalsList(goals: Goal[]) { save('rj_goals_list', goals) }
export function addGoal(g: Omit<Goal, 'id' | 'createdAt'>) {
  const goals = getGoalsList()
  const goal: Goal = { ...g, id: `g_${Date.now()}_${Math.random().toString(36).slice(2, 7)}`, createdAt: new Date().toISOString() }
  saveGoalsList([...goals, goal])
  return goal
}
export function updateGoal(id: string, patch: Partial<Goal>) {
  saveGoalsList(getGoalsList().map(g => g.id === id ? { ...g, ...patch } : g))
}
