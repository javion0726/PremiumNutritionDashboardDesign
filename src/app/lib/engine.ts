...
// ─── weekly plan progression (V2) ────────────────────────────────────────────
// A plan's schedule is a fixed 7-slot Mon–Sun array. currentDayIdx points into
// that array; finishing a training day (or skipping it) advances the index,
// wrapping to the next week when it runs past Sunday, and the plan is
// considered complete once currentWeek exceeds totalWeeks.

export function advancePlanDay(current: { currentWeek: number; currentDayIdx: number }, scheduleLength: number, totalWeeks: number): { currentWeek: number; currentDayIdx: number; completed: boolean } {
  let { currentWeek, currentDayIdx } = current
  currentDayIdx += 1
  if (currentDayIdx >= scheduleLength) {
    currentDayIdx = 0
    currentWeek += 1
  }
...
export function resolveGoalCurrent(linkedMetric: 'weight' | 'streak' | 'bodyFat' | 'manual', storedCurrent: number): number {
  if (linkedMetric === 'manual') return storedCurrent
  if (linkedMetric === 'streak') return calcStreak()
...
// ─── recovery signal (rule-based from what's logged today, no invented data) ──

export function recoveryScore(k: string = todayKey(), j: Journal = getJournal()): { score: number; hasInputs: boolean; notes: string[] } {
  const cfg = getConfig()
  const e = j[k]
  let score = 70
  const notes: string[] = []
  ...
  return { score: Math.max(10, Math.min(98, score)), hasInputs, notes }
}
